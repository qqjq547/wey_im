 ...
 * INFO:通讯录 adapter
 */
@Route(path = Constant.ARouter.ROUNTE_SERVICE_ADAPTER_CHATS, name = "搜索个人消息")
class SearchChatsAdapter : BaseSearchAdapter<MultiItemEntity, BaseViewHolder>(null) {
                        ...
                        mMapList.forEach {
                            if (it[SEARCH_USER_ID] == item.ownerUid.toString()) {
                                ...
                                helper.itemView.setOnClickListener {
                                    EventBus.publishEvent(SearchChatEvent(CHAT_TYPE_PVT, item.id, mTargetId))
                                }
                                return@outside
                            }
                        }
