...
class SelectNameCardContactsAdapter : AppBaseMultiItemQuickAdapter<MultiItemEntity, BaseViewHolder>(null), SectionIndexer {

    init {
        addItemType(0, R.layout.msg_name_card_item)
        ...
        item?.let {
            if (item.itemType == 0) {
                ...
                if (helper?.itemView?.context?.getString(R.string.string_star).equals(itemData)){
                    ...
                    helper?.getView<TextView>(R.id.text_view_head_name)?.text = itemData
                }
            }
        }
    }
