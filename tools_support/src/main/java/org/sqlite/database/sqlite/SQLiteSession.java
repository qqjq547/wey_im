...
/**
 * Provides a single client the ability to use a org.sqlite.database.
 *
 * <h2>About org.sqlite.database sessions</h2>
 * <p>
 * Database access is always performed using a session.  The session
 * manages the lifecycle of transactions and org.sqlite.database connections.
 * </p><p>
 ...
 * </p><p>
 * When <em>Write Ahead Logging (WAL)</em> is enabled, the org.sqlite.database can
 ...
 * of other threads.
 * </p><p>
 * A thread has at most one session per org.sqlite.database.  This constraint ensures that
 * a thread can never use more than one org.sqlite.database connection at a time for a
 * given org.sqlite.database.  As the number of available org.sqlite.database connections is limited,
 * if a single thread tried to acquire multiple connections for the same org.sqlite.database
 * at the same time, it might deadlock.  Therefore we allow there to be only
 * one session (so, at most one connection) per thread per org.sqlite.database.
 * </p>
 *
 * <h2>Transactions</h2>
 * <p>
 ...
 * transactions.
 * </p><p>
 * An implicit transaction is created whenever a org.sqlite.database operation is requested
 * and there is no explicit transaction currently in progress.  An implicit transaction
 * only lasts for the duration of the org.sqlite.database operation in question and then it
 * is ended.  If the org.sqlite.database operation was successful, then its changes are committed.
 * </p><p>
 ...
 * all subsequent org.sqlite.database operations will be performed as part of that transaction.
 ...
 * {@link #yieldTransaction}.  If there is contention for use of the org.sqlite.database,
 * then yielding ends the current transaction, commits its changes, releases the
 * org.sqlite.database connection for use by another session for a little while, and starts a
 ...
 * time.  Each session acquires and releases connections to the org.sqlite.database
 * as needed to perform each requested org.sqlite.database transaction.  If all connections
 * are in use, then org.sqlite.database transactions on some sessions will block until a
 * connection becomes available.
 * </p><p>
 * The session acquires a single org.sqlite.database connection only for the duration
 * of a single (implicit or explicit) org.sqlite.database transaction, then releases it.
 * This characteristic allows a small pool of org.sqlite.database connections to be shared
 * efficiently by multiple sessions as long as they are not all trying to perform
 * org.sqlite.database transactions at the same time.
 * </p>
 *
 * <h2>Responsiveness</h2>
 * <p>
 * Because there are a limited number of org.sqlite.database connections and the session holds
 * a org.sqlite.database connection for the entire duration of a org.sqlite.database transaction,
 ...
 * Guidelines:
 * <ul>
 * <li>Do not perform org.sqlite.database transactions on the UI thread.</li>
 * <li>Keep org.sqlite.database transactions as short as possible.</li>
 * <li>Simple queries often run faster than complex queries.</li>
 * <li>Measure the performance of your org.sqlite.database transactions.</li>
...
public final class SQLiteSession {
     ...
     * <p>
     * In a deferred transaction, no locks are acquired on the org.sqlite.database
     ...
     * it is possible for another session to write to the org.sqlite.database first before
     ...
     * or write but no other sessions are allowed to access the org.sqlite.database.
     * </p><p>
    ...
    /**
     * Returns true if the session has an active org.sqlite.database connection.
     *
     * @return True if the session has an active org.sqlite.database connection.
     */
     ...
     * then a org.sqlite.database connection is obtained and a new transaction is started.
     ...
     * the org.sqlite.database.  Begins a new transaction after a specified delay.
     * <p>
     * If there are other threads waiting to acquire connections,
     * then the current transaction is committed and the org.sqlite.database
     ...
     * the org.sqlite.database connection to allow other threads some time to run.
