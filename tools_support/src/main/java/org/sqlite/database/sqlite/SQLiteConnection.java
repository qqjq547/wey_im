...
/**
 * Represents a SQLite org.sqlite.database connection.
 * Each connection wraps an instance of a native <code>sqlite3</code> object.
 * <p>
 * When org.sqlite.database connection pooling is enabled, there can be multiple active
 * connections to the same org.sqlite.database.  Otherwise there is typically only one
 * connection per org.sqlite.database.
 * </p><p>
 * When the SQLite WAL feature is enabled, multiple readers and one writer
 * can concurrently access the org.sqlite.database.  Without WAL, readers and writers
 ...
 * perform a org.sqlite.database operation and are then returned to the pool.  At any
...
public final class SQLiteConnection implements CancellationSignal.OnCancelListener {
    ...
    // Called by SQLiteConnectionPool only.
    // Closes the org.sqlite.database closes and releases all of its associated resources.
    ...
    private void setJournalMode(String newValue) {
        ...
        if (!value.equalsIgnoreCase(newValue)) {
            try {
                ...
                // This error (SQLITE_BUSY) occurs if one connection has the org.sqlite.database
                // open in WAL mode and another tries to change it to non-WAL.
            }
            // Because we always disable WAL mode when a org.sqlite.database is first opened
            // (even if we intend to re-enable it), we can encounter problems if
            // there is another open connection to the org.sqlite.database somewhere.
            // This can happen for a variety of reasons such as an application opening
            // the same org.sqlite.database in multiple processes at the same time or if there is a
            ...
            Log.w(TAG, "Could not change the org.sqlite.database journal mode of '"
                    + mConfiguration.label + "' from '" + value + "' to '" + newValue
                    + "' because the org.sqlite.database is locked.  This usually means that "
                    + "there are other open connections to the org.sqlite.database which prevents "
                    + "the org.sqlite.database from enabling or disabling write-ahead logging mode.  "
    ...
    private void setLocaleFromConfiguration() {
        ...
        // If the org.sqlite.database is read-only, we cannot modify the android metadata table
        // or existing indexes.
        if (mIsReadOnlyConnection) {
            return;
        }
    ...
    /**
     * Returns true if this is the primary org.sqlite.database connection.
     * @return True if this is the primary org.sqlite.database connection.
     */
    ...
    private void throwIfStatementForbidden(PreparedStatement statement) {
        if (mOnlyAllowReadOnlyOperations && !statement.mReadOnly) {
            throw new SQLiteException("Cannot execute this statement because it "
                    + "might modify the org.sqlite.database but the connection is read-only.");
        }
    }
...
/*        }*/
    ...
    /**
     * Collects statistics about org.sqlite.database connection memory usage.
     *
     * @param dbStatsList The list to populate.
     */
    void collectDbStats(ArrayList<DbStats> dbStatsList) {
        // Get information about the main org.sqlite.database.
        ...
        // We ignore the first row in the org.sqlite.database list because it corresponds to
        // the main org.sqlite.database which we have already described.
    ...
    /**
     * Collects statistics about org.sqlite.database connection memory usage, in the case where the
     * caller might not actually own the connection.
     *
    ...
    private DbStats getMainDbStatsUnsafe(int lookaside, long pageCount, long pageSize) {
        ...
        // even if we do not own the org.sqlite.database connection.
     ...
     * the native org.sqlite.database object can be closed.  So no finalizers here.
     */
