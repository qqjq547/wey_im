package framework.telegram.support.tools.wordfilter;
...
public class WordFilter {
    ...
    public static void addSensitiveWords(String keywords) {
        try {
            addSensitiveWord(Arrays.asList(keywords.split(",")));
        } catch (Exception e) {
        }
    }

    public static void addDetaultSensitiveWords() {
        try {
     ...
     * @return
     */
    private static List<String> readWordFromFile(String fileName) {
        BufferedReader br = null;
        try {
            br = new BufferedReader(new InputStreamReader(BaseApp.app.getAssets().open(fileName)));
            String keywords = IoUtils.readAllChars(br);
