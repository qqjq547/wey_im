...
public class IntentUtils {
    ...
    public static Intent openPlayStore(Context context, boolean openInBrowser) {
        ...
        if (openInBrowser) {
            return openLink("https://play.google.com/store/apps/details?id="
                    + appPackageName);
        }
        return marketIntent;
    }
     ...
     */
    public static Intent showStreetView(float latitude, float longitude,
                                        Float yaw, Integer pitch, Float zoom, Integer mapZoom) {
     ...
     */
    public static Intent showLocation(float latitude, float longitude,
                                      Integer zoomLevel) {
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_VIEW);
    ...
    /**
     * Open a apk file in appropriate app
     *
     * @param file File to open
     */
    public static Intent openApk(String file) {
        return openApk(new File(file));
    }
    ...
    public static Intent openApk(Uri uri) {
        return openMedia(uri, "application/vnd.android.package-archive");
    }
     ...
     * @see #cropImage(Context, File, int, int, int,
     * int, boolean)
     */
     ...
     */
    public static Intent cropImage(Context context, File image, int outputX,
                                   int outputY, int aspectX, int aspectY, boolean scale) {
        Intent intent = new Intent("com.android.camera.action.CROP");
        intent.setType("image/*");
    ...
    public static boolean isIntentAvailable(Context context, Intent intent) {
        ...
        // fixd由于系统定制的厂商根据自己的需求对系统电话应用的包名（Google默认为：com.android.contacts）进行了修改，导致通过Action为Intent.ACTION_DIAL的Intent跳转不能直接进入系统拨号界面
        if (list != null && !list.isEmpty() && Intent.ACTION_DIAL.equals(intent.getAction())) {
            List<String> tmps = new ArrayList<>();
            tmps.add("com.android.contacts"); // 系统默认
            tmps.add("com.sec.android.app.dialertab"); // 三星
            tmps.add("com.sonyericsson.android.socialphonebook"); // 索爱
            tmps.add("com.sonyericsson.phone"); //索爱1.6系统
            tmps.add("com.android.htcdialer"); // htc
            tmps.add("com.android.phone"); //LG GT540
            tmps.add("com.motorola.dialer"); //Moto A953
            tmps.add("com.yulong.android.contacts.dial"); //酷派
            tmps.add("com.zte.smartdialer"); //中兴N880
            tmps.add("com.modoohut.dialer"); //三星i9000

            for (ResolveInfo resolveInfo : list) {
                String packname = resolveInfo.activityInfo.packageName;
                if (tmps.contains(packname)) {
