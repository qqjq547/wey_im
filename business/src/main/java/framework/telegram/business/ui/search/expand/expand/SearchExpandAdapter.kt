...
import framework.telegram.business.bridge.Constant.ARouter.ROUNTE_BUS_SEARCH_CHAT_EXPAND
...
import framework.telegram.business.bridge.Constant.Search.SEARCH_MATCH_COUNT
 ...
 * INFO:通讯录 adapter
 */
class SearchExpandAdapter : BaseMultiItemQuickAdapter<MultiItemEntity, BaseViewHolder>(null) {

    init {
        addItemType(SEARCH_MERGE_CHAT, R.layout.bus_search_chat_item)
        addItemType(SEARCH_ITEM_TITLE, R.layout.bus_title2)
        addItemType(ContactDataModel.CONTACT_ITEM_TYPE, R.layout.bus_search_contacts_item_2)
        ...
        when (item?.itemType) {
            ...
            ContactDataModel.CONTACT_ITEM_TYPE -> {
                if (item is ContactDataModel) {
                    ...
                    helper.itemView.setOnClickListener {
                        ARouter.getInstance().build(Constant.ARouter.ROUNTE_MSG_PRIVATE_CHAT_ACTIVITY).withLong("targetUid", item.uid).navigation()
                    }
                }
            }
            SEARCH_ITEM_GROUP -> {
            ...
            SearchMergeChatModel.SEARCH_MERGE_CHAT -> {
                if (item is SearchMergeChatModel) {
                    ...
                    helper.itemView.setOnClickListener {
                        ...
                        ARouter.getInstance().build(ROUNTE_BUS_SEARCH_CHAT_EXPAND)
                                .withString(KEYWORD, mKeyword)
                                .withBoolean(SEARCH_BACK,true)
                                .withInt(SEARCH_MATCH_COUNT,item.matchCount)
                                .withString(TARGET_NAME,targetName)
                                .withLong(INDEX_ID,item.indexId).navigation()
                    }
                }
            }
        }
