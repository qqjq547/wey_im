package framework.telegram.ui.videoplayer.video;
...
import framework.telegram.ui.videoplayer.listener.GSYVideoShotListener;
import framework.telegram.ui.videoplayer.listener.GSYVideoShotSaveListener;
import framework.telegram.ui.videoplayer.utils.Debuger;
import framework.telegram.ui.videoplayer.utils.NetworkUtils;
import framework.telegram.ui.videoplayer.video.base.GSYBaseVideoPlayer;
import framework.telegram.ui.videoplayer.video.base.GSYVideoPlayer;

import java.io.File;

import framework.telegram.ui.videoplayer.view.DownloadView;
import framework.telegram.ui.videoplayer.view.PlayView;
...
//        //去掉SeekBar 的ontouch 事件
//        if (mProgressBar!=null){
//            mProgressBar.setOnTouchListener(null);
//        }
//
//        //去掉整个界面 的ontouch 事件（亮度，音量）
//        mContainer = findViewById(R.id.surface_container);
//        if (mContainer != null){
...
//        }
        ...
        if (closeView!=null){
            closeView.setOnClickListener(v -> {
                if (mIVideoClick!=null)
                    mIVideoClick.onCloseClick();
            });
        }

        View moreView = findViewById(R.id.image_more);
        if (closeView!=null){
            moreView.setOnClickListener(v -> {
                if (mIVideoClick!=null)
                    mIVideoClick.onMoreClick();
            });
        }

        View goView = findViewById(R.id.image_go);
        if (goView!=null){
            goView.setOnClickListener(v -> {
                if (mIVideoClick!=null)
                    mIVideoClick.onGoClick();
            });
        }
    }
...
//        setViewShowState(mThumbImageViewLayout, VISIBLE);
        ...
        updateStartImage();
        if (mLoadingProgressBar instanceof DownloadView) {
            ((DownloadView) mLoadingProgressBar).reset();
        }
    }
    ...
    protected void changeUiToPreparingShow() {
        ...
        if (mLoadingProgressBar instanceof DownloadView) {
            DownloadView downloadView = (DownloadView) mLoadingProgressBar;
            if (downloadView.getCurrentState() == DownloadView.STATE_PRE) {
                ((DownloadView) mLoadingProgressBar).start();
            }
        }
    }
...
//        setViewShowState(mThumbImageViewLayout, VISIBLE);
    ...
    private float viewToSourceX(float vx) {
        ...
        return (vx - vTranslate.x);
    }

    private float viewToSourceY(float vy) {
        if (vTranslate == null) { return Float.NaN; }
        return (vy - vTranslate.y);
    }
    ...
    public void showCompleteUi(boolean isShow){
        if (isShow){
            changeUiToCompleteShow();
        }else {
            changeUiToCompleteClear();
    ...
    private IVideoClick mIVideoClick;

    public void setVideoClick(IVideoClick click){
       mIVideoClick = click;
    }

    public void setVisiableMoreView(Boolean isVisable){
        View moreView = findViewById(R.id.image_more);
        if (moreView !=null){
            moreView.setVisibility(isVisable?View.VISIBLE : View.GONE);
        }
        View goView = findViewById(R.id.image_go);
        if (goView !=null){
            goView.setVisibility(isVisable?View.VISIBLE : View.GONE);
        }
    }

    public View getBarLayout(){
        return findViewById(R.id.bar_layout);
    }
    ...
    //给下拉动画用的 v1.6.0 by lin  ----------上面面--------
    public int setVolumeState(int volume){
    ...
    @Override
    public void onAutoCompletion() {//为了解决播放完了后,不是停在最后一帧的bug
