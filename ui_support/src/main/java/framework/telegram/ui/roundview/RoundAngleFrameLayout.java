package framework.telegram.ui.roundview;
...
public class RoundAngleFrameLayout extends FrameLayout {
    ...
    private void drawBorder(Canvas canvas) {
        if (isShowBorder) {
            ...
            canvas.drawLine(bottomLeftRadius, getHeight() - 1, getWidth() - bottomRightRadius, getHeight() - 1, borderPaint);
    ...
    private void drawBottomLeft(Canvas canvas) {
        if (bottomLeftRadius > 0) {
            ...
            path.arcTo(new RectF(0, height - 2 * bottomLeftRadius, bottomLeftRadius * 2, height - 1), 90, 90);
            path.close();
            canvas.drawPath(path, roundPaint);
            if (isShowBorder) {
                Path path1 = new Path();
                path1.arcTo(new RectF(0, height - 2 * bottomLeftRadius, bottomLeftRadius * 2, height - 1), 90, 90);
                canvas.drawPath(path1, borderPaint);
            }
        }
    }
    ...
    private void drawBottomRight(Canvas canvas) {
        if (bottomRightRadius > 0) {
            ...
            path.arcTo(new RectF(width - 2 * bottomRightRadius, height - 2 * bottomRightRadius, width, height - 1), 0, 90);
            path.close();
            canvas.drawPath(path, roundPaint);
            if (isShowBorder) {
                Path path1 = new Path();
                path1.arcTo(new RectF(width - 2 * bottomRightRadius, height - 2 * bottomRightRadius, width, height - 1), 0, 90);
                canvas.drawPath(path1, borderPaint);
            }
        }
    }
