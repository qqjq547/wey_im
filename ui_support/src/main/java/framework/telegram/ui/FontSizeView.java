...
public class FontSizeView extends View {
    ...
    private int bigSize = 24;

    // 圆半径
    private int circleRadius;
    private int circleColor = Color.parseColor("#178aff");
    // 一段的宽度，根据总宽度和总格数计算得来
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // initDefault
        paddingLR = ScreenUtils.dp2px(context,30);
        defaultLineWidth = ScreenUtils.dp2px(context, 2);
        defaultCircleRadius = ScreenUtils.dp2px(context, 35);
        lineWidth = ScreenUtils.dp2px(context, 1);
...
//        mTextPaint.setStrokeWidth(ScreenUtils.dp2px(context, 1));
        mTextPaint.setTextSize(ScreenUtils.sp2px(context, smallSize));
        textScaleX = mTextPaint.measureText(getContext().getString(R.string.small));
        //文字画笔
        ...
        mText1Paint.setTextSize(ScreenUtils.sp2px(context, bigSize));
        text1ScaleX = mText1Paint.measureText(getContext().getString(R.string.big));

        //文字画笔
        mText2Paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mText2Paint.setColor(Color.parseColor("#9B9B9B"));
        mText2Paint.setStyle(Paint.Style.FILL_AND_STROKE);
        mText2Paint.setTextSize(ScreenUtils.sp2px(context, standerSize));
        text2ScaleX = mText2Paint.measureText(getContext().getString(R.string.standard));
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        ...
        } else if (attr == R.styleable.FontSizeView_lineWidth_2) {
            lineWidth = typedArray.getDimensionPixelSize(attr, defaultLineWidth);
        } else if (attr == R.styleable.FontSizeView_circleRadius_2) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        circleY = height*7/11;
        // 横线宽度是总宽度-2个圆的半径
        itemWidth = (w - 2 * circleRadius-paddingLR*2) / max;
        // 把可点击点保存起来
        for (int i = 0; i <= max; i++) {
            points.add(new Point(circleRadius + i * itemWidth+paddingLR, height*7/11));
        }
        //初始刻度
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        //画字
        canvas.drawText(getContext().getString(R.string.small), points.get(0).x - textScaleX / 2, height*7/11 - 50, mTextPaint);

        //画字
        canvas.drawText(getContext().getString(R.string.standard), points.get(1).x - text2ScaleX / 2, height*7/11 - 50, mText2Paint);

        //画字
        canvas.drawText(getContext().getString(R.string.big), points.get(points.size() - 1).x - text1ScaleX / 2, height*7/11 - 50, mText1Paint);

        // 先画中间的横线
        canvas.drawLine(points.get(0).x, height*7/11, points.get(points.size() - 1).x, height*7/11, mLinePaint);
        // 绘制刻度
        for (Point point : points) {
            canvas.drawLine(point.x + 1, height*7/11 - 20, point.x + 1, height*7/11 , mLinePaint);
        }

        // 画圆
        if (currentX < circleRadius+paddingLR) {
            currentX = circleRadius+paddingLR;
        }
        if (currentX > width - circleRadius-paddingLR) {
            currentX = width - circleRadius-paddingLR;
        }
