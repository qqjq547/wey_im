package framework.telegram.ui.swipememu;
...
public class AppSwipeMenuLayout extends ViewGroup {

    private static final String TAG = "AppSwipeMenuLayout";
    ...
    private static AppSwipeMenuLayout mViewCache;
    private static State mStateCache;
    private float distanceX;
    private float finalyDistanceX;

    public AppSwipeMenuLayout(Context context) {
        this(context, null);
    }

    public AppSwipeMenuLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AppSwipeMenuLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //1、获取配置的属性值
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.AppSwipeMenuLayout, defStyleAttr, 0);

        try {
            int indexCount = typedArray.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int attr = typedArray.getIndex(i);
                if (attr == R.styleable.AppSwipeMenuLayout_leftMenuView) {
                    mLeftViewResID = typedArray.getResourceId(R.styleable.AppSwipeMenuLayout_leftMenuView, -1);
                } else if (attr == R.styleable.AppSwipeMenuLayout_rightMenuView) {
                    mRightViewResID = typedArray.getResourceId(R.styleable.AppSwipeMenuLayout_rightMenuView, -1);
                } else if (attr == R.styleable.AppSwipeMenuLayout_contentView) {
                    mContentViewResID = typedArray.getResourceId(R.styleable.AppSwipeMenuLayout_contentView, -1);
                } else if (attr == R.styleable.AppSwipeMenuLayout_canLeftSwipe) {
                    mCanLeftSwipe = typedArray.getBoolean(R.styleable.AppSwipeMenuLayout_canLeftSwipe, true);
                } else if (attr == R.styleable.AppSwipeMenuLayout_canRightSwipe) {
                    mCanRightSwipe = typedArray.getBoolean(R.styleable.AppSwipeMenuLayout_canRightSwipe, true);
                } else if (attr == R.styleable.AppSwipeMenuLayout_fraction) {
                    mFraction = typedArray.getFloat(R.styleable.AppSwipeMenuLayout_fraction, 0.5f);
                }
            }
...
//                }
    ...
    public static AppSwipeMenuLayout getViewCache() {
        return mViewCache;
    }
