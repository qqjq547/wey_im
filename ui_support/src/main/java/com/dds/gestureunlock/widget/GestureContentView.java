...
public class GestureContentView extends FrameLayout {
     ...
     */
    public GestureContentView(Context context, boolean isVerify, String passWord, long errorMaxTime,
        ...
        addChild(data);
        // 初始化一个可以画线的view
        gestureDrawline = new GestureDrawLine(context, list, isVerify, passWord, errorMaxTime,
    ...
    private void addChild(ConfigGestureVO data) {
        for (int i = 0; i < 9; i++) {
            CircleImageView image = new CircleImageView(context, circleWidth,
                    ...
                    data.getSelectedThemeColor(),
                    data.getErrorThemeColor(),data.getUnSelectColor1(),data.getUnSelectColor2(),data.getLineColor(), data.isShowTrack());
            this.addView(image);
            invalidate();
            // 第几行
            int row = i / 3;
    ...
    public void setErrorMaxTime(long errorMaxTime){
        gestureDrawline.setErrorMaxTime(errorMaxTime);
    }
